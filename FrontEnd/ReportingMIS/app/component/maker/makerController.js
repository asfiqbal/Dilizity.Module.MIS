...
(function () {
    ...
    makerController.$inject = ['$scope', '$stateParams', '$rootScope', 'AuthenticationService', 'UniversalService', 'Notification'];
    function makerController($scope, $stateParams, $rootScope, AuthenticationService, UniversalService, Notification) {
        var vm = this;
        ...
        //vm.load = load;
        vm.deleteMakerActivities = deleteMakerActivities;
              ...
              { name: 'ActionUrl', field: 'ActionUrl' },
              {
                  name: 'Action', cellTemplate: '<div>' +
                                                '<ul class="nav navbar-right panel_toolbox">'+
	                                                '<li>'+
                                                		'<a uib-tooltip="View Role" tooltip-placement="left" ui-sref="{{row.entity.ActionUrl}}" class="pull-right"><i class="fa fa-eye"></i></a>' +
	                                                '</li>'+
	                                                '<li>'+
		                                                '<a uib-tooltip="Edit Role" tooltip-placement="left" ui-sref="{{row.entity.ActionUrl}}" class="pull-right"><i class="fa fa-pencil-square"></i></a>' +
	                                                '<li>' +
                                                '</ul>'+
                                               '</div>'
              }
        ...
        (function initController() {
            console.log("makerController.initController -> Begin");

            permissionId = $stateParams.permissionId;
            console.log("permissionId", permissionId);

            loadScreenPermissionsAndInfo();
            console.log("makerController.initController -> End");
        })();
        ...
        function deleteMakerActivities() {
            console.log("deleteMakerActivities Begin");
            angular.element('#modal').modal('hide');
            var tmpUserName = $rootScope.globals.currentUser.username;
            var deletePermission = vm.permissionName + '.Delete';
            console.log("permissionName", vm.permissionName);
            console.log("vm.selectedMakerActivities", vm.selectedMakerActivities);

            var model = {
                Roles: vm.selectedMakerActivities
            }
            ...
            console.log("deleteMakerActivities End");
