...
{
    class CheckerScreenManager : IAbstractBusiness
    {
        private const string CHECKER_SCREEN_USER_PERMISSION = "CheckerScreenUserPermission";
        ...
        public void Do(BusService parameterBusService)
        {
            using (FnTraceWrap tracer = new FnTraceWrap())
            {
                ...
                using (DynamicDataLayer dataLayer = new DynamicDataLayer(GlobalConstants.SECURITY_SCHEMA))
                {
                    ...
                    foreach (dynamic permission in dataLayer.ExecuteUsingKey(CHECKER_SCREEN_USER_PERMISSION, GlobalConstants.LOGIN_PARAM, LoginId, GlobalConstants.PERMISSION_PARAM, permissionId))
                    {
                        if (permission.FieldKey == "Checker")
