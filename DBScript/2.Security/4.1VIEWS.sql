...
AS
SELECT  DISTINcT U.LOGIN_ID, RP.PERMISSION_ID,P.PARENT_PERMISSION_ID, P.PERMISSION_NAME
FROM	SEC_USER U
...
WITH WF ( Permission_id, PARENT_PERMISSION_ID, PERMISSION_NAME  )
AS
(
SELECT	Permission_id, PARENT_PERMISSION_ID , P.PERMISSION_NAME
from	SEC_PERMISSION P
WHERE   P.PARENT_PERMISSION_ID is null
UNION ALL
SELECT	P.Permission_id, P.PARENT_PERMISSION_ID, P.PERMISSION_NAME
from	SEC_PERMISSION P
		INNER JOIN WF ON P.PARENT_PERMISSION_ID = WF.Permission_id 
)
SELECT PERMISSION_ID, PARENT_PERMISSION_ID,  PERMISSION_NAME, dbo.FN_GET_FRIENDLY_PERMISSION_NAME(PERMISSION_NAME) USER_FRIENDLY_NAME
FROM	WF
